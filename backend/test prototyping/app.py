...
def process_file():
    ...
    try:
        ...
        dstats = descriptive_stats()
        ct = convert_dict_dtypes(dstats.central_tendency(cleaned_df))
        var = convert_dict_dtypes(dstats.variability(cleaned_df))
        dist = convert_dict_dtypes(dstats.distribution(cleaned_df))
        shapeNspread = convert_dict_dtypes(dstats.shapeNspread(cleaned_df))
        rel = convert_dict_dtypes(dstats.relationships_mvar(cleaned_df))
        freq_stats = convert_dict_dtypes(dstats.freq_analysis(cleaned_df))
        dataset_description = convert_dict_dtypes(dstats.dataset_desc(cleaned_df))
